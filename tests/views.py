...
def create_test(request, module_id):
    ...
    if not professor.is_staff:
        return Response({"error": "Você não tem permissão para criar testes."}, status=status.HTTP_403_FORBIDDEN)
    ...
    if not test_data or not questions_data:
        return Response({"error": "Dados do teste ou perguntas estão faltando."}, status=status.HTTP_400_BAD_REQUEST)
    ...
    try:
        ...
        return Response({"message": "Teste criado com sucesso!"}, status=status.HTTP_201_CREATED)
...
def submit_test_answers(request, test_id):
    ...
    if student.is_staff:
        return Response({"error": "Professors are not allowed to submit test answers."}, status=status.HTTP_403_FORBIDDEN)
    ...
    try:
        ...
        for answer_data in answers_data:
            try:
                ...
                return Response({"error": f"Question with id {answer_data['question_id']} not found."}, status=status.HTTP_404_NOT_FOUND)
