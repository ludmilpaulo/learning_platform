...
def create_content(request, module_id):
    ...
    try:
        ...
        return Response(
            {"detail": "Módulo não encontrado."}, status=status.HTTP_404_NOT_FOUND
        )
    ...
    try:
        ...
        return Response(
            {"detail": "Tipo de conteúdo inválido."}, status=status.HTTP_400_BAD_REQUEST
        )
    ...
    if content_type_name == "text":
        ...
        Content.objects.create(
            module=module, content_type=content_type, object_id=text_content.id
        )
        ...
        video_content = Video.objects.create(
            owner=user, title=request.data.get("title", "Conteúdo de Vídeo"), url=url
        )
        Content.objects.create(
            module=module, content_type=content_type, object_id=video_content.id
        )
        ...
        Content.objects.create(
            module=module, content_type=content_type, object_id=image_content.id
        )
        ...
        Content.objects.create(
            module=module, content_type=content_type, object_id=file_content.id
        )
    ...
    return Response(
        {"detail": "Conteúdo criado com sucesso!"}, status=status.HTTP_201_CREATED
    )
