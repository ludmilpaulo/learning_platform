...
)  # Allow unauthenticated users, but we'll manually authenticate
def add_module_to_course(request, course_id):
    ...
    try:
        ...
        return Response(
            {"status": "failed", "error": "Invalid or missing access token."},
    ...
    try:
        ...
        return Response(
            {"status": "failed", "error": "Course not found."},
    ...
    if course.owner != user:
        return Response(
            {
                "status": "failed",
                "error": "You are not authorized to add a module to this course.",
            },
