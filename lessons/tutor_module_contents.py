...
def get_module_contents(request, module_id):
    ...
    try:
        ...
        return Response({"error": "Token inválido ou ausente."}, status=status.HTTP_401_UNAUTHORIZED)
    ...
    if module.course.owner != user and not module.course.students.filter(id=user.id).exists():
        raise PermissionDenied("Você não tem permissão para visualizar o conteúdo deste módulo.")
